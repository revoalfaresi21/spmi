...
export default function Footer() {
  return (
    <footer className="relative bg-[#07101a] border-t border-white/5 pt-20 pb-8 mt-24 overflow-hidden">
      ...
      <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-3/4 h-px bg-gradient-to-r from-transparent via-cyan-500/50 to-transparent"></div>
      <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-1/4 h-1 bg-gradient-to-r from-transparent via-cyan-400 to-transparent blur-sm"></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8 mb-16">
          ...
          <div className="lg:col-span-3">
            ...
            <ul className="space-y-4">
              {['Beranda', 'Profil', 'Siklus PPEPP', 'Dokumen Mutu', 'Audit Internal (AMI)'].map((item, i) => (
                <li key={i}>
                  <Link to={item === 'Beranda' ? '/' : `/${item.toLowerCase().replace(/ /g, '-').replace(/[()]/g, '')}`} className="text-sm text-slate-400 hover:text-cyan-400 transition-colors flex items-center gap-2 group">
            ...
            <ul className="space-y-5">
              <li className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 leading-relaxed">Jl. Tuparev, Cirebon, Jawa Barat</p>
                </div>
              </li>
